...
namespace STATION_Utils {
    ...
    void sendBeacon() {
        if (sendStartTelemetry && ((Config.battery.sendVoltage && Config.battery.voltageAsTelemetry) || (Config.telemetry.sendTelemetry)) && lastTxTime > 0) TELEMETRY_Utils::sendEquationsUnitsParameters();
        ...
        if (!shouldSleepLowVoltage) {
            ...
            int sendCommentAfterXBeacons = ((Config.battery.sendVoltageAlways) ? 1 : Config.sendCommentAfterXBeacons);
            ...
            if (comment != "" || (Config.battery.sendVoltage && Config.battery.voltageAsTelemetry) || (Config.telemetry.sendTelemetry)) {
                updateCounter++;
                if (updateCounter >= sendCommentAfterXBeacons) {
                    if (comment != "") packet += comment;
                    if ((Config.battery.sendVoltage && Config.battery.voltageAsTelemetry) || (Config.telemetry.sendTelemetry)) packet += TELEMETRY_Utils::generateEncodedTelemetry();
                    updateCounter = 0;
                }
            }
        } else {
        ...
        // Agregado de mediciones de sensor al packet.
        String sensorMessage = SENSORS_Utils::getAllReadings();
